...
pub async fn get_image(id: &str) -> Result<Vec<f32>> {
	let image = reqwest::ClientBuilder::new()
		.user_agent(
			"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) \
			 Chrome/111.0.0.0 Safari/537.36",
		)
		.build()?
		.get(format!("https://imgs.hcaptcha.com/{id}"))
		.send()
		.await?
		.error_for_status()?
		.bytes()
	...
	tokio::task::spawn_blocking(move || -> Result<Vec<f32>> {
		...
		let out = Vec::with_capacity(IMAGE_DIM as usize * IMAGE_DIM as usize);
		let (mut r, g, b) = image
			.into_rgb8()
			.pixels()
			...
			.fold(
				...
				|(mut r, mut g, mut b), [cr, cg, cb]| {
					// even though the hcaptcha-solver swaps the first and last channel
					// we don't need to as we don't load the image with swapped channels
					r.push(cr);
					g.push(cg);
					b.push(cb);
					(r, g, b)
